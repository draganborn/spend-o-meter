...
export async function handler(event) {
  ...
  const { google_sub, range, values } = JSON.parse(event.body);
  if (!google_sub || !range || !values) {
    return { statusCode: 400, body: JSON.stringify({ error: 'Missing google_sub, range, or values' }) };
  }
  ...
  try {
    ...
    return {
      statusCode: 500,
      body: JSON.stringify({ error: 'Failed to write sheet', details: error.message }),
    };
  }
}
